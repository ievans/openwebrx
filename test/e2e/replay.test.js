...
// All of these tests need more history than the 12s IQ buffer configured
// for this server, which only comes from real wall-clock time passing. They
// share one page and build it up once, instead of each paying for its own
// ~15s of dead air.
describe('replay, once enough history has built up', () => {
    let page;
    before(async () => {
        page = await h.openReceiver(browser);
